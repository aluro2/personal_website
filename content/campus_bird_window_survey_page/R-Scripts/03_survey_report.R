...
bird_data %>%
  ...
  ggtitle("Birds Found by Building Face Direction") +
  theme_fivethirtyeight() +
  theme(#panel.background = element_rect(fill = "black",
    #colour = "black",
    #size = 0.5, linetype = "solid"),
...
ggsave("plot_building_face_birds_found.png", dpi = 600, width = 10, height = 8, device = "png")



# Campus map from Google maps -----------------------------------------------------
...
# Static map of birds found across campus buildings -----------------------

## Map with select building labels (> 20 birds found)
ggmap(UIUC) +
  ...
  scale_fill_gradient2(low = "blue",
                       ...
                       labels = seq(25, 200, 25)) +
  labs(fill = "Number of \n Window-Killed Birds",
       subtitle = "Survey conducted in September-November of 2019-2020") +
  geom_label_repel(data = subset(bird_data_map, n_birds > 20),
                   aes(x = lon,
                       y = lat,
                       label = building_name),
                   force = 15,
...
# Interactive leaflet map of birds found at campus buildings --------------
